...
- If a file contains exactly one primary class or struct, the file basename must
  match that type name in `CamelCase` / `PascalCase`
  (for example `HttpServerModule.hpp` for `HttpServerModule`).
...
- Header files must start with `#pragma once`; if an include guard is also used, prefer a `HTTP_SERVER_*_HPP_INCLUDED` style guard that matches the file.
