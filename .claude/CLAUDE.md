...
Required sequence (state machine — never repeat a step):

> Anti-loop directive: within a single request, calling the same Codebase Memory tool with the same arguments more than once is forbidden. If `index_status` returns `ready`, do NOT call it again — proceed directly to discovery tools.
...
2. Call `mcp__codebase-memory__index_status` exactly once.
   - If `ready`: immediately proceed to step 3. NEVER repeat this call.
   - If `not found` / `not indexed`: call `list_projects` once, identify the correct project name, then call `index_status` once with that name. If still not ready, call `index_repository` once. Do NOT loop back to step 2.
3. If the project is indexed, use ONE of:
   ...
   Do not call `index_status` again after this step.
4. Only if Codebase Memory is unavailable or fails after retry, fall back to
   Glob/Grep/Read/LSP.
...
- Project code lives only under `src/`; do not create or restore a top-level `include/` directory.
- Public headers live under `src/http_server/...`, with the umbrella header at `src/http_server.hpp`.
- Keep matching `.hpp` and `.cpp` files side by side inside the nearest `src/http_server/...` subdirectory.
- CMake exposes `src` as the include root, so consumer code should keep using `#include <http_server.hpp>` or `#include <http_server/...>`.
