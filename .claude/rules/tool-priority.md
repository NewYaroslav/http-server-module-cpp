...
- Codebase Memory loop prevention: if `index_status` returns `ready`, do NOT call it again. A second `index_status` on the same project in the same request is a loop — proceed to `search_graph`/`trace_path`/`get_code_snippet` immediately.
...
Preferred sequence (state machine — no repeats):

1. `mcp__codebase-memory__index_status` once.
   - `ready` → go to step 2.
   - `not found` / `not indexed` → `list_projects` once, then `index_status` once with the correct name. If still not ready → `index_repository` once. Do NOT return to step 1.
...
Hard anti-loop guard: within a single reasoning chain, the same Codebase Memory tool MUST NOT be called twice with identical parameters. After `index_status` returns `ready`, any subsequent `index_status` call is a loop — abort and proceed to `search_graph`, `trace_path`, `get_architecture`, or `get_code_snippet`.
